 ...
 *   posljednje = { "glavna": [[id,type,name,datetime,temp,rr,ws,wdir,gust], ...], "klimatoloska": [...], "padavinska": [...] }
 *   stanice    = [[id, wmo, lat, lon, elev, name, type, active], ...]
 *
 * Missing values arrive as "" — never coerce 0 to null.
 */
...
export function parseLocalMeasuredAt(local: string): string | null {
  ...
  // Montenegro is UTC+1 (CET) or UTC+2 (CEST). Approximate with fixed offset
  // for Slice 1; refine with proper TZ lib later if needed.
  // Aug = CEST → UTC+2
...
/** Extract JS assignment `name=...` from HTML (object or array). */
...
function simpleHash(input: string): string {
  // FNV-1a 32-bit — enough to detect source change
  let h = 2166136261;
...
export async function fetchAndParseAwsLatest(
  ...
  const url = baseUrl || Deno.env.get("ZHMS_BASE_URL")
    ? `${(baseUrl || Deno.env.get("ZHMS_BASE_URL")!).replace(/\/$/, "")}/Meteorologija/aws_m.php`
    : DEFAULT_URL;
  ...
  const res = await fetch(url, {
    headers: {
      "User-Agent":
        "weather-mne-rokg/1.0 (personal weather app; contact via github.com/forkusis)",
      Accept: "text/html",
    },
  });
  if (!res.ok) {
  ...
  // Evaluate-safe parse: source is our controlled extract of array/object literals
  ...
  for (const key of Object.keys(posljednje)) {
    ...
    for (const row of group) {
      ...
      // Basic sanity: reject absurd temperatures
